...
def get_dataset_model_path(data_path: str, default_model_path: str = "data/model_params") -> str:
    ...
    # 否则创建目录（只要数据名不是默认的 "data"）
    if os.path.exists(dataset_dir):
        os.makedirs(dataset_dir, exist_ok=True)
        return os.path.join(dataset_dir, "model_params")
    elif name_without_ext not in ("data", "exported_data"):
        os.makedirs(dataset_dir, exist_ok=True)
        return os.path.join(dataset_dir, "model_params")
    else:
...
def load_jsonl(filepath: str, extract_features: bool = True,
               max_per_voltage: int = 10000) -> list[dict]:
    """
    从 JSONL 文件加载所有记录，并自动清洗 ACTUAL_VOLTAGE 字段为 float。

    支持 max_per_voltage 参数：每个电压（按 10V 分桶）最多保留 N 条，
    超出部分随机丢弃，防止某个电压样本过多造成数据不平衡。

    Args:
        filepath: JSONL 文件路径
        extract_features: 是否预提取时域统计特征（Vpp/Kurtosis/Skewness），
                          数据增强等场景可设为 False 加快速度。
        max_per_voltage: 每个电压桶最多保留条数（默认 10000，0 表示不限制）
    ...
    if max_per_voltage > 0:
        ...
        for rec in records:
            ...
            if v is None:
                continue
            bucket = round(abs(float(v)) / 10) * 10
            buckets.setdefault(bucket, []).append(rec)

        # 如果没有电压信息（未知数据），直接返回全部记录
        if not buckets:
            return records
        ...
        for bucket in sorted(buckets):
            ...
            if len(group) > max_per_voltage:
                random.shuffle(group)
                group = group[:max_per_voltage]
