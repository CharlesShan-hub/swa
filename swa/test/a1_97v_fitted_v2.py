"""
97V 的 A1 / 湿度 / 温度 随时间变化的散点 + 拟合曲线（10阶）。
"""
import sys, os
...
xs, ys = poly_fit(times, a1s)
ax1.plot(xs, ys, "-", linewidth=2.5, color="#D32F2F", label=f"拟合曲线 ({DEG}阶)")
ax1.set_ylabel("A1")
...
xs, ys = poly_fit(times, hums)
ax2.plot(xs, ys, "-", linewidth=2.5, color="#D32F2F", label=f"拟合曲线 ({DEG}阶)")
ax2.set_ylabel("湿度 (%)")
...
xs, ys = poly_fit(times, temps)
ax3.plot(xs, ys, "-", linewidth=2.5, color="#D32F2F", label=f"拟合曲线 ({DEG}阶)")
ax3.set_ylabel("温度 (°C)")
...
plt.tight_layout()
plt.savefig("test/a1_97v_fitted_10deg.png", dpi=150)
plt.close()
print(f"已保存: test/a1_97v_fitted_10deg.png（{DEG}阶，{len(data)} 条）")
